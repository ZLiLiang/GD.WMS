<template>
    <el-dialog title="库存选择框" v-model="open" width="900px" :show-close="false" :close-on-click-modal="false">
        <div class="container">
            <div class="left-form">
            ...
            <div class="right-table">
                <!-- 表格 -->
                <el-table v-loading="loading" :data="stockList" highlight-current-row @row-click="onRowClick" height="100%">
                    <el-table-column width="55" align="center" label="选择">
                        <template #default="scope">
                            <el-radio :label="scope.row.stockId" v-model="selectRadio">{{ ' ' }}</el-radio>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column label="可用数量" prop="qty" />
                    <template #append>
                        <!-- 分页栏 -->
...
<script setup>
import { getStockSelect } from '@/api/inventory/stock'
...
function getList() {
    ...
    getStockSelect(params).then(res => {
        loading.value = false
