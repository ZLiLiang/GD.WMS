    ...
    <div class="app-container">
        ...
        <el-form :model="queryParams" ref="queryRef" v-show="showSearch" :inline="true">
            <el-form-item label="货主名称" prop="ownerName">
                <el-input v-model="queryParams.ownerName" placeholder="请输入货主名称" clearable style="width: 160px" />
            </el-form-item>
        ...
        <!-- 表格 -->
        <el-table v-loading="loading" :data="ownerList" highlight-current-row>
            <el-table-column label="货主名称" prop="ownerName" v-if="columns.showColumn('ownerName')" />
...
const upload = reactive({
    ...
    // 上传的地址
    url: import.meta.env.VITE_APP_BASE_API + "/warehousemanagement/owner/importData",
});
// 数据
const data = reactive({
    form: {
    ...
    rules: {
        ownerName: [{ required: true, message: '货主名称不能为空', trigger: 'blur' }],
...
// 列显隐信息
const columns = ref([
    { key: 0, label: `货主名称`, visible: true, prop: 'ownerName' },
    { key: 1, label: `所在城市`, visible: true, prop: 'city' },
    { key: 2, label: `详细地址`, visible: true, prop: 'address' },
    { key: 3, label: `联系方式`, visible: true, prop: 'contactTel' },
    { key: 4, label: `负责人`, visible: true, prop: 'manager' },
    { key: 5, label: `创建人`, visible: false, prop: 'createBy' },
...
function getList() {
    ...
    getAllInfo(params).then(res => {
        loading.value = false
        ownerList.value = res.data.result
    ...
    proxy.$refs['ownerRef'].validate((valid) => {
        if (valid) {
            if (form.value.ownerId != undefined) {
                editInfo(form.value.ownerId, form.value).then(res => {
...
function handleDelete(row) {
    ...
    proxy
        .$modal
        .confirm('是否确认删除"' + row.ownerName + '"的数据项？')
        .then(() => {
            return deleteInfo(ownerId)
        })
        .then(() => {
            getList()
