    ...
    <div class="app-container">
        ...
        <el-form :model="queryParams" ref="queryRef" v-show="showSearch" :inline="true">
            ...
            <el-form-item label="所在城市" prop="city">
                <el-input v-model="queryParams.city" placeholder="请输入所在城市" clearable style="width: 160px" />
            </el-form-item>
        ...
        <!-- 表格 -->
        <el-table v-loading="loading" :data="warehouseList" highlight-current-row>
        ...
        <el-dialog :title="title" v-model="open" width="600px" :show-close="false" :draggable="true">
            <el-form :model="form" :rules="rules" ref="warehouseRef" label-width="80px">
                <el-form-item label="仓库名称" prop="warehouseName">
...
const upload = reactive({
    ...
    // 上传的地址
    url: import.meta.env.VITE_APP_BASE_API + "/warehousemanagement/warehouse/importData",
});
// 数据
const data = reactive({
    form: {},
...
const columns = ref([
    ...
    { key: 5, label: `负责人`, visible: true, prop: 'manager' },
    ...
    proxy.$refs['warehouseRef'].validate((valid) => {
        if (valid) {
            if (form.value.warehouseId != undefined) {
                editInfo(form.value.warehouseId, form.value).then(res => {
...
function handleUpdate(row) {
    ...
    getInfo(warehouseId).then(response => {
...
function handleDelete(row) {
    ...
    proxy
        .$modal
        .confirm('是否确认删除"' + row.warehouseName + '"的数据项？')
        .then(() => {
            return deleteInfo(warehouseId)
        })
        .then(() => {
            getList()
