<template>
    <div class="app-container">
        ...
        <el-form :model="queryParams" ref="queryRef" v-show="showSearch" :inline="true">
            <el-form-item label="联系方式" prop="locationCode">
                <el-input v-model="queryParams.locationCode" placeholder="请输入库位名称" clearable style="width: 160px" />
            </el-form-item>
        ...
        <!-- 表格 -->
        <el-table v-loading="loading" :data="locationStockList" highlight-current-row>
            <el-table-column label="仓库名称" prop="warehouseName" />
            <el-table-column label="库位名称" prop="locationCode" />
...
<script setup>
import { getLocationStock, exportLocationStock } from "@/api/inventory/stock";
...
function getList() {
    ...
    getLocationStock(params).then(res => {
        loading.value = false
        locationStockList.value = res.data.result
...
function handleExport() {
    proxy
        .$modal
        .confirm('是否确认导出所有库位列表数据项?', '警告', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        })
        .then(async () => {
            await exportLocationStock()
        })
}
