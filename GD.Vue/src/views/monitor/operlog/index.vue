        ...
        <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
            ...
            <el-form-item label="业务类型" prop="businessType">
                <el-select v-model="queryParams.businessType" placeholder="操作类型" clearable>
                    <el-option v-for="dict in options.businessTypeOptions" :key="dict.Value" :label="dict.Label"
                        :value="dict.Value" />
                </el-select>
            </el-form-item>
            <el-form-item label="操作状态" prop="status">
                <el-select v-model="queryParams.status" placeholder="操作状态" clearable>
                    <el-option v-for="dict in options.statusOptions" :key="dict.Value" :label="dict.Label"
                        :value="dict.Value" />
                </el-select>
            </el-form-item>
            ...
            <el-form-item label="操作时间">
                <el-date-picker v-model="dateRange" type="datetimerange" range-separator="-" start-placeholder="开始日期"
                    end-placeholder="结束日期" value-format="YYYY-MM-DD HH:mm:ss" :shortcuts="dateRange"></el-date-picker>
            </el-form-item>
            <el-form-item>
        ...
        <el-table v-loading="loading" border :data="list" @selection-change="handleSelectionChange">
            ...
            <el-table-column prop="businessType" label="业务类型" align="center" v-if="columns.showColumn('businessType')">
                <template #default="scope">
                    {{ options.businessTypeOptions[scope.row.businessType].Label }}
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作状态" align="center" prop="status" v-if="columns.showColumn('status')">
                <template #default="{ row }">
                    <el-tag :disable-transitions="true" type="success"
                        v-if="options.statusOptions[row.status].Value === '0'">{{ options.statusOptions[row.status].Label
                        }}</el-tag>
                    <el-tag :disable-transitions="true" type="danger" v-else>{{ options.statusOptions[row.status].Label
                    }}</el-tag>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="140">
                <template #default="scope">
                    ...
                    <el-button size="small" text icon="delete" @click="handleDelete(scope.row)"> 删除 </el-button>
                </template>
            </el-table-column>
        </el-table>
                ...
                <el-row>
                    ...
                    <el-col :lg="12">
                        <el-form-item label="操作类型：">
                            {{ options.businessTypeOptions[form.businessType].Label }}
                        </el-form-item>
                    </el-col>
                    <el-col :lg="12">
                        <el-form-item label="操作状态：">
                            <el-tag :disable-transitions="true" type="success"
                                v-if="options.statusOptions[form.status].Value === '0'">{{
                                    options.statusOptions[form.status].Label }}</el-tag>
