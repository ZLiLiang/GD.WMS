<template>
    <div class="app-container">
        ...
        <!-- 表格 -->
        <el-table v-loading="loading" :data="actualList" highlight-current-row>
        ...
        <el-dialog v-model="open" title="编辑上架数量" :show-close="false" width="300px">
            <el-form :model="form" :rules="rules" ref="asnNoticeRef">
                <el-form-item label="库位编码" prop="locationId">
                    <el-input v-model="form.locationId" placeholder="请输入库位编码" clearable @click="inputClick"
                        :readonly="true" />
                </el-form-item>
                <el-form-item label="上架数量" prop="putawayQty">
                    <el-input v-model="form.putawayQty" placeholder="请输入上架数量" clearable />
                </el-form-item>
            </el-form>
...
function getList() {
    ...
    getAllInfo(params).then(res => {
        loading.value = false
        actualList.value = res.data.result
